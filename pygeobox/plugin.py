...
PLUGINS = {
    'api_backend': {
        'Elasticsearch': {
            'plugin': 'pygeobox.api.backend.elastic.ElasticBackend'
        },
        'SensorThings': {
            'plugin': 'pygeobox.api.backend.sensorthings.SensorthingsBackend'
        }
    },
    'api_config': {
        'pygeoapi': {
            'plugin': 'pygeobox.api.config.pygeoapi.PygeoapiConfig'
        }
    },
    'storage': {
        'S3': {
            'plugin': 'pygeobox.storage.minio.MinIOStorage'
        }
    }
}
...
def load_plugin(plugin_type: PluginTypes, defs: dict) -> Any:
    ...
    if plugin_type in ['api_backend', 'api_config', 'storage']:
